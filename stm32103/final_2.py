...
def send_packet_data_periodically():
    ...
    window.after(1, send_packet_data_periodically)  # 每 1ms 发送一次
...
def update_joystick_positions():
    ...
    if not input_active:
        ...
        yaw_entry.insert(0, f"{remoterData['yaw']:.1f}")
        thrust_entry.delete(0, tk.END)
        thrust_entry.insert(0, f"{remoterData['thrust']:.1f}")
        roll_entry.delete(0, tk.END)
        roll_entry.insert(0, f"{remoterData['roll']:.1f}")
        pitch_entry.delete(0, tk.END)
        pitch_entry.insert(0, f"{remoterData['pitch']:.1f}")
...
def apply_values():
    try:
        # 获取并转换输入值
        yaw_val = float(yaw_entry.get())
        thrust_val = float(thrust_entry.get())
        roll_val = float(roll_entry.get())
        pitch_val = float(pitch_entry.get())
        
        # 数值范围校验
        if not (-200 <= yaw_val <= 200):
            raise ValueError("Yaw超出范围")
        if not (0 <= thrust_val <= 100):
            raise ValueError("Thrust超出范围") 
        if not (-50 <= roll_val <= 50):
            raise ValueError("Roll超出范围")
        if not (-50 <= pitch_val <= 50):
            raise ValueError("Pitch超出范围")
            
        # 反向计算摇杆位置（映射到-2~2范围）
        left_stick['x'] = (yaw_val - (-200)) / (200 - (-200)) * (2 - (-2)) + (-2)
        left_stick['y'] = (thrust_val - 0) / (100 - 0) * (2 - (-2)) + (-2)
        right_stick['x'] = (roll_val - (-50)) / (50 - (-50)) * (2 - (-2)) + (-2)
        right_stick['y'] = (pitch_val - (-50)) / (50 - (-50)) * (2 - (-2)) + (-2)
...
# 创建输入框和标签
yaw_label_input = tk.Label(input_frame, text="Yaw (-200~200):")
yaw_label_input.grid(row=0, column=0)
yaw_entry = tk.Entry(input_frame)
yaw_entry.grid(row=0, column=1)

thrust_label_input = tk.Label(input_frame, text="Thrust (0~100):")
thrust_label_input.grid(row=1, column=0)
thrust_entry = tk.Entry(input_frame)
thrust_entry.grid(row=1, column=1)

roll_label_input = tk.Label(input_frame, text="Roll (-50~50):")
roll_label_input.grid(row=2, column=0)
roll_entry = tk.Entry(input_frame)
roll_entry.grid(row=2, column=1)

pitch_label_input = tk.Label(input_frame, text="Pitch (-50~50):")
pitch_label_input.grid(row=3, column=0)
pitch_entry = tk.Entry(input_frame)
pitch_entry.grid(row=3, column=1)

apply_button = tk.Button(input_frame, text="执行", command=apply_values)
apply_button.grid(row=4, columnspan=2, pady=5)

# 在创建输入框后添加事件绑定
for entry in [yaw_entry, thrust_entry, roll_entry, pitch_entry]:
