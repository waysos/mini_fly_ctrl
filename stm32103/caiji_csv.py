...
# 创建csv文件并写入表头
csv_file = open('caiji_values.csv', mode='w', newline='')
csv_writer = csv.writer(csv_file)
csv_writer.writerow(['Time','roll','pitch','yaw', 'altitude', 'a_x', 'a_y', 'a_z', 'v_x', 'v_y', 'v_z', 'q_x', 'q_y', 'q_z'])
...
def record_to_csv():
    ...
    csv_writer.writerow([current_time, roll_z, pitch_z, yaw_z, altitude_z, a_x_c, a_y_c, a_z_c, v_x_c, v_y_c, v_z_c, q_x_c, q_y_c, q_z_c])
    csv_file.flush()  # 每次写入后刷新文件
    root.after(20, record_to_csv)  # 每100ms记录一次
