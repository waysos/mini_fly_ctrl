...
def main():
    ...
    # 发送起飞命令
    send_packet_CMD(ser, CMD_FLIGHT_LAND)
    
    time.sleep(2)  # 等待一段时间
    
    # 设置控制数据
    remoterData['roll'] = 20.0 # 设置roll为5°
    remoterData['thrust'] = 50.0 # 设置thrust为50
    remoterData['ctrlMode'] = 1 # 设置控制模式为定高
    
    # 调用函数：1秒内，每0.001秒发送一次数据包
    send_data_for_duration(ser, duration=1, interval=0.001)
